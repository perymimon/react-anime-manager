import {Meta, Preview, Story, Canvas, ArgsTable, Source} from '@storybook/addon-docs/blocks';
import './stories/stories-style.css'
...
<Meta title="Hooks/useAnimeManager"
...
- [API](#api-useanimemanager-exposed-by-perymimonreact-anime-manager)
...
The hook `useAnimeManager` design to be placed between the data that create the JSX and the JSX result.
It got the tracking data in one side and provide `stateItems` array that contain the same date plus data from the past that come with essential info tells
for each datum what it is relative to previous update.
...
<Canvas>
    <Story name="test" id="examples--position-vs-location"/>
</Canvas>
