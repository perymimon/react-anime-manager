import {ADD, SWAP, REMOVE, STATIC, useAnimeManager} from '../anime-manager.js'
...
test('test array items phases [MOVE]', async function (){
    ...
    rerender([10,20,30])
    expect(result.current).toMatchObject([
        {item: 10, phase: ADD, from:Infinity, to:0},
        {item: 20, phase: ADD, from:Infinity, to:1},
        {item: 30, phase: ADD, from:Infinity, to:2}
    ])
